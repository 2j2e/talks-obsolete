...
module.exports = {
  ...
  , 'Create Server instance': function () {
    ...
    // dummy socket to emulate a `real` socket
      , dummySocket = {
        readyState: 'open', end: function (buffer) {
          calls++;
    ...
    var port = 1335
      , httpserver = http.createServer(function (q, r) {
        r.writeHead(200);
        r.end(':3')
      })
    ...
    var port = 1345
      , ssl = {
        key: fs.readFileSync(__dirname + '/ssl/ssl.private.key').toString(), cert: fs.readFileSync(__dirname + '/ssl/ssl.crt').toString()
      }
      , httpserver = https.createServer(ssl, function (q, r) {
        r.writeHead(200);
        r.end(':3')
      })
      , server = fspfs.createServer();
