...
#Region "ToDgml"
    ...
    Public Function ToDgml(nodeOrToken As CommonSyntaxNodeOrToken, language As String,
        ...
        If nodeOrToken.IsNode Then
            dgml = ToDgml(nodeOrToken.AsNode, language, options)
        Else
            dgml = ToDgml(nodeOrToken.AsToken, language, options)
        End If
    ...
    Public Function ToDgml(node As CommonSyntaxNode, language As String,
        ...
        ProcessNode(options, node, language, dgml)
        Return dgml
    End Function
    ...
    Public Function ToDgml(token As CommonSyntaxToken, language As String,
        ...
        Dim dgml = GetDgmlTemplate(options)
        ProcessToken(options, token, language, dgml)
        Return dgml
    End Function
    ...
    Public Function ToDgml(trivia As CommonSyntaxTrivia, language As String,
        ...
        Dim dgml = GetDgmlTemplate(options)
        ProcessTrivia(options, trivia, language, dgml)
        Return dgml
    End Function
#End Region
...
#Region "Process*"
    Private Sub ProcessNodeOrToken(options As SyntaxDgmlOptions, nodeOrToken As CommonSyntaxNodeOrToken,
        ...
        If nodeOrToken.IsNode Then
            ProcessNode(options, nodeOrToken.AsNode, language, dgml, count, parent, parentGroup, properties)
        Else
            ProcessToken(options, nodeOrToken.AsToken, language, dgml, count, parent, parentGroup, properties)
        End If
    End Sub
    ...
    Private Sub ProcessNode(options As SyntaxDgmlOptions, node As CommonSyntaxNode,
        ...
        AddNodeInfo(options, node, language, current, dgml, properties)
        ...
        If options.ShowGroups Then
            ...
            AddNodeInfo(options, node, language, currentGroup, dgml, properties)
        ...
        For Each childSyntaxNode In node.ChildNodesAndTokens()
            ProcessNodeOrToken(options, childSyntaxNode, language, dgml, count, current, currentGroup, properties)
        Next
    End Sub
    ...
    Private Sub ProcessToken(options As SyntaxDgmlOptions, token As CommonSyntaxToken,
        ...
        AddTokenInfo(options, token, language, current, dgml, properties)
        Dim currentGroup As XElement = parentGroup
        ...
        If options.ShowTrivia Then
            For Each triviaNode In token.LeadingTrivia
                ProcessTrivia(options, triviaNode, language, dgml, count, True, current, currentGroup, properties)
            Next
            For Each triviaNode In token.TrailingTrivia
                ProcessTrivia(options, triviaNode, language, dgml, count, False, current, currentGroup, properties)
            Next
        End If
    End Sub
    ...
    Private Sub ProcessTrivia(options As SyntaxDgmlOptions, trivia As CommonSyntaxTrivia,
        ...
        Initialize(options, dgml, parent, parentGroup, current, properties, count, 0, 0, 0)
        AddTriviaInfo(options, trivia, language, current, dgml, properties)
        Dim currentGroup As XElement = parentGroup
        ...
        If options.ShowTrivia Then
            If trivia.HasStructure Then
                ProcessNode(options, trivia.GetStructure, language, dgml, count, current, currentGroup, properties)
            End If
        End If
    End Sub
#End Region
